...
parser.add_argument(
    "-f", "--file", default="E:\data\imagenet_ai_0424_sdv5", type=str, help="path to data directory"
)
...
parser.add_argument("--detector_path", type=str, default="data/detector-ckpt/finetune_0_.pkl")
parser.add_argument(
    "-m",
    "--model_path",
    type=str,
...
def main():
    data = dataset.ProcessorDataset("E:/data/my_sd", opt.img_size)
        ...
        for i, imgs in enumerate(train_loader):
            ...
            with torch.no_grad():
                ...
                for j in range(opt.num):
                    ...
                    weighted_direction += t_noise / opt.num
            ...
            if i % 100 == 0:
                ...
                print(
                    "epoch: {}, i: {}\nprob1: {}, prob2: {}, best_prob3: {}, avg_prob3: {}\nloss: {}, ssim: {}, t: {}s".format(
                        epoch, i, prob1.mean(), prob2.mean(), best_prob3.mean(), avg_prob3.mean(), loss.item(), ssim,
                        tt - t
                    ))
            ...
            if i % 1148 == 0 and (epoch != 0 or i != 0):
                checkpoint = {"model_state_dict": p_net.state_dict(),
                              ...
                              "epoch": epoch}
                path_checkpoint = "data/processor-ckpt/train_by_my_sd_{}_{}_.pkl".format(epoch, i)
